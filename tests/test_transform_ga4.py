...
def test_classify_source_channel_custom_channels_full():
    ...
    df = pd.DataFrame(
        [
            {"channel": "Referral", "medium": "referral", "source": "extra.shiseido.co.jp"},
    ...
    result = classify_source_channel(
        ...
        custom_channels={
            "Shiseido Internal": {
                "normalize": {},
                "detect": [r"extra\.shiseido\.co\.jp", r"sharepoint"]
            }
        }
    )
    
    assert result["channel"].tolist() == ["Shiseido Internal", "Shiseido Internal", "Referral"]
