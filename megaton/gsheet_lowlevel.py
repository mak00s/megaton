"""Low-level stateless gspread / Sheets batchUpdate helpers.

The STATELESS entry style for Google Sheets (spreadsheet id + service-account
path, one-shot calls) — the shared implementation home for both megaton's
stateful clients and downstream consumers (promoted from megaton-app
``megaton_lib/gspread_lowlevel.py`` in megaton 2.1.0; that module is now a
re-export shim).

Retry-safe network calls use :func:`call_with_retry` (exponential backoff;
HTTP 429 and quota-style 403s wait at least 30 seconds; nested calls collapse
into the outer retry loop). Non-idempotent append/insert/delete/add calls are
deliberately submitted once to avoid duplicate mutations after a lost
response.
"""
